...
class ReplayBuffer(Dataset):
    ...
    def __init__(self, obs_shape, action_shape, capacity, batch_size, device,image_size=84,transform=None,is_latent=False,num_copies=1):
        # self.num_envs = 1
        # if len(obs_shape) == 4: # if obs_shape is (num_envs, channel, h, w)
        #     capacity //= obs_shape[0]
    ...
    def add(self, obs, action, reward, next_obs, done): 
        for i in range(obs.shape[0]):
            ...
            np.copyto(self.not_dones[self.idx], 1-done[i])
    ...
    def sample_cpc(self, random_crop):

        start = time.time()
        idxs = np.random.randint( # TODO: change sampling to sample both idx and env_num
            0, self.capacity if self.full else self.idx, size=self.batch_size
        )
...
def random_crop(imgs, output_size):
    ...
    imgs = imgs.reshape((n*num_envs, *imgs.shape[2:]))
    crop_max = img_size - output_size
    imgs = np.transpose(imgs, (0, 2, 3, 1))
    w1 = np.random.randint(0, crop_max, n*num_envs) # crop_max is 0 which is not allowed TODO
    h1 = np.random.randint(0, crop_max, n*num_envs)
